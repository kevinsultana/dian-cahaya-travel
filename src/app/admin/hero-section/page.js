...
export default function HeroSectionPage() {
  ...
  const [formData, setFormData] = useState({ judul: "", subjudul: "", gambar: "", link_tombol: "/paket-umrah", status: "aktif" });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      Swal.fire({ icon: "error", title: "Error", text: "Gagal memuat data hero section." });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openAddModal = () => {
    setEditingId(null);
    setFormData({ judul: "", subjudul: "", gambar: "", link_tombol: "/paket-umrah", status: "aktif" });
    setIsModalOpen(true);
  };

  const openEditModal = (item) => {
    setEditingId(item.id);
    setFormData({ judul: item.judul || "", subjudul: item.subjudul || "", gambar: item.gambar || "", link_tombol: item.link_tombol || "/paket-umrah", status: item.status || "aktif" });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      let res;
      if (editingId) {
        res = await fetch(`/api/hero-section/${editingId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(formData) });
      } else {
        res = await fetch("/api/hero-section", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(formData) });
      }
      if (!res.ok) throw new Error();
      setIsModalOpen(false);
      await fetchData();
      Swal.fire({ icon: "success", title: "Berhasil!", text: editingId ? "Slide hero berhasil diperbarui." : "Slide hero berhasil ditambahkan.", timer: 1500, showConfirmButton: false });
    } catch {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Apakah Anda yakin?", text: "Slide hero ini akan dihapus secara permanen!", icon: "warning",
      showCancelButton: true, confirmButtonColor: "#d33", cancelButtonColor: "#3085d6",
      confirmButtonText: "Ya, Hapus!", cancelButtonText: "Batal",
    }).then(async (result) => {
      if (result.isConfirmed) {
        try {
          const res = await fetch(`/api/hero-section/${id}`, { method: "DELETE" });
          if (!res.ok) throw new Error();
          await fetchData();
  ...
  return (
    <div>
      <AdminHeader title="Homepage" onLogout={() => { localStorage.removeItem("adminLoggedIn"); router.push("/admin/login"); }} />

      <main className="p-8">
        <div className="flex justify-between items-center mb-6">
          <p className="text-sm text-on-surface-variant">Kelola slide banner utama/Homepage pada halaman depan</p>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-surface-container-low text-left">
                    ...
                    <th className="px-6 py-3 text-xs font-semibold text-on-surface-variant uppercase tracking-wider">Link Tombol</th>
                ...
                <tbody className="divide-y divide-outline-variant">
                  {data.map((item) => (
                    <tr key={item.id} className="hover:bg-surface-container-low transition-colors">
                      <td className="px-6 py-4 text-sm text-on-surface">
                        {item.gambar ? (
                          <div className="w-16 h-10 rounded bg-cover bg-center" style={{ backgroundImage: `url(${item.gambar})` }} />
                        ) : (
                          <span className="text-xs text-on-surface-variant">No Image</span>
                        )}
                      </td>
                      <td className="px-6 py-4 text-sm font-medium text-primary max-w-xs truncate">{item.judul}</td>
                  ...
                  {data.length === 0 && (
                    <tr><td colSpan="5" className="px-6 py-8 text-center text-sm text-on-surface-variant">Belum ada slide hero. Klik &quot;Tambah Slide Hero&quot; untuk membuat baru.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? "Edit Slide Hero" : "Tambah Slide Hero"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-semibold text-primary mb-2">Judul Utama (Title)</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-primary mb-2">Subjudul (Subtitle)</label>
          ...
          <div>
            <label className="block text-sm font-semibold text-primary mb-2">Gambar Latar Belakang (URL)</label>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-primary mb-2">Link Tombol CTA</label>
              <input type="text" value={formData.link_tombol} onChange={(e) => setFormData({ ...formData, link_tombol: e.target.value })} className="w-full bg-surface border border-outline-variant rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary outline-none transition-all text-sm" required />
            </div>
            <div>
