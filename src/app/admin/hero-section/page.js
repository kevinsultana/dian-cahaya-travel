...
export default function HeroSectionPage() {
  ...
  const [data, setData] = useLocalStorage("adminHeroSection", initialHeroSection);
  ...
  const openEditModal = (item) => {
    ...
    setFormData({
      ...
      gambar: item.gambar || "",
      link_tombol: item.link_tombol || "/paket-umrah",
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const newItem = {
      id: editingId || `hs-${Date.now()}`,
      judul: formData.judul,
      subjudul: formData.subjudul,
      gambar: formData.gambar,
      link_tombol: formData.link_tombol,
    ...
    if (editingId) {
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
        text: "Slide hero berhasil diperbarui.",
        timer: 1500,
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil!",
        text: "Slide hero berhasil ditambahkan.",
        timer: 1500,
        showConfirmButton: false,
      });
    }
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      text: "Slide hero ini akan dihapus secara permanen!",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          icon: "success",
          title: "Terhapus!",
          text: "Slide hero berhasil dihapus.",
          timer: 1500,
          showConfirmButton: false,
        });
      }
  ...
  return (
    <div>
                ...
                {data.length === 0 && (
                  <tr>
                    <td colSpan="5" className="px-6 py-8 text-center text-sm text-on-surface-variant">
                      Belum ada slide hero. Klik "Tambah Slide Hero" untuk membuat baru.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      <Modal
        ...
        title={editingId ? "Edit Slide Hero" : "Tambah Slide Hero"}
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                value={formData.link_tombol}
                onChange={(e) => setFormData({ ...formData, link_tombol: e.target.value })}
                className="w-full bg-surface border border-outline-variant rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary outline-none transition-all text-sm"
                required
              />
            </div>
