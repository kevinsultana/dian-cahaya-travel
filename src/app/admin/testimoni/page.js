...
export default function TestimoniPage() {
  ...
  const [formData, setFormData] = useState({ nama: "", foto: "", bintang: 5, teks_deskripsi: "" });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      Swal.fire({ icon: "error", title: "Error", text: "Gagal memuat data testimoni." });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setSaving(true);
    const payload = { ...formData, bintang: Number(formData.bintang) };

    try {
      let res;
      if (editingId) {
        res = await fetch(`/api/testimoni/${editingId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      } else {
        res = await fetch("/api/testimoni", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      }
      if (!res.ok) throw new Error();
      setIsModalOpen(false);
      await fetchData();
      Swal.fire({ icon: "success", title: "Berhasil!", text: editingId ? "Testimoni berhasil diperbarui." : "Testimoni berhasil ditambahkan.", timer: 1500, showConfirmButton: false });
    } catch {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      title: "Apakah Anda yakin?", text: "Testimoni ini akan dihapus secara permanen!", icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          ...
          if (!res.ok) throw new Error();
          await fetchData();
          Swal.fire({ icon: "success", title: "Terhapus!", text: "Testimoni berhasil dihapus.", timer: 1500, showConfirmButton: false });
        } catch {
  ...
  return (
    <div>
      <AdminHeader title="Testimoni" onLogout={() => { localStorage.removeItem("adminLoggedIn"); router.push("/admin/login"); }} />

      <main className="p-8">
        <div className="flex justify-between items-center mb-6">
          <p className="text-sm text-on-surface-variant">Kelola testimoni jamaah</p>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-outline-variant">
                    ...
                    <tr key={item.id} className="hover:bg-surface-container-low transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex text-secondary">
                          {[...Array(5)].map((_, i) => (
                            <span key={i} className="material-symbols-outlined text-[16px]" style={{ fontVariationSettings: "'FILL' 1", color: i < item.bintang ? "#735c00" : "#ccc" }}>star</span>
                          ))}
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-on-surface line-clamp-1 max-w-xs">{item.teks_deskripsi}</td>
                      <td className="px-6 py-4">
                  ...
                  {data.length === 0 && (
                    <tr><td colSpan="5" className="px-6 py-10 text-center text-sm text-on-surface-variant">Belum ada data testimoni.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? "Edit Testimoni" : "Tambah Testimoni"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <textarea value={formData.teks_deskripsi} onChange={(e) => setFormData({ ...formData, teks_deskripsi: e.target.value })} className="w-full bg-surface border border-outline-variant rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-primary outline-none transition-all text-sm h-32 resize-none" required />
          </div>
